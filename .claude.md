...
- **Check at session start**: At the beginning of each session, check if repo configuration (`.claude/commands/`) is newer than local `~/.claude/commands/` and suggest syncing if updates are available
